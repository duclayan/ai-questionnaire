...
export const MermaidDiagram = ({
  ...
  async function processMermaidCode(code) {
    if (
      !code ||
      code.toLowerCase().includes("we need more information".toLowerCase())
    ) {
      return null;
    }
    ...
    try {
      ...
      for (let i = 0; i < diagramPrompts.length; i++) {
        try {
          ...
          generated_chart = await processMermaidCode(
      ...
      setErrorMessage(
        "Seems like I don't know how to do that yet. You can try rephrasing it or just click generate  again."
      );
      setSaveGraph(false);
      ...
      if (!isRetry) {
        try {
          let count = 0;
          let corrected_chart = await handleMermaidError(
            generated_chart,
            error.message
          );
          while (count < 3) {
            corrected_chart = await handleMermaidError(
              generated_chart,
              error.message
            );
            if (corrected_chart) {
  ...
  const handleMermaidError = async (generated_chart, error_message) => {
    setLoading(true);
    setErrorMessage(
      "Please Hold On. This might take a moment. Your patience is greatly appreciated"
    );
  ...
  return (
    ...
    >
      <div style={{ width: "100%", textAlign: "center" }}>
        ...
        {loading && (
          <DocumentLoader isLoading={loading} text={"Preparing the Data"} />
        )}
